...
public class UbicacionController implements Initializable {
    ...
    @FXML private TableView<UbicacionPercha> tblUbicacion;
    @FXML private TableColumn<UbicacionPercha, Integer> colId;
    @FXML private TableColumn<UbicacionPercha, String> colNombre;
    @FXML private TableColumn<UbicacionPercha, String> colEstado;
    @FXML private TableColumn<UbicacionPercha, Void> colAcciones;
    ...
    private void cargarDatos() {
        listaUbicacion.setAll(dao.listar());
    }
    ...
    private void guardar() {
        try {
            ...
            logDAO.guardar("UbicacionController", "guardar", e.getMessage(), e);
            new Alert(Alert.AlertType.ERROR, "Error al guardar: " + e.getMessage()).showAndWait();
        }
    }

    private void cargarFormulario(UbicacionPercha u) {
    ...
    private void filtroBusqueda() {
        FilteredList<UbicacionPercha> filtered = new FilteredList<>(listaUbicacion, p -> true);
        tblUbicacion.setItems(filtered);
        txtBuscar.textProperty().addListener((obs, old, val) -> {
            ...
            filtered.setPredicate(u -> u.getNombre().toLowerCase().contains(texto));
        });
    }
        ...
        colAcciones.setCellFactory(param -> new TableCell<>() {
            ...
            private final HBox hbox = new HBox(10);
            {
                ...
                btnEliminar.setOnAction(e -> {
                    UbicacionPercha u = getTableView().getItems().get(getIndex());
                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "¿Eliminar esta ubicación?", ButtonType.YES, ButtonType.NO);
                    if (alert.showAndWait().orElse(ButtonType.NO) == ButtonType.YES) { dao.eliminar(u.getId()); cargarDatos(); }
                });
            }
