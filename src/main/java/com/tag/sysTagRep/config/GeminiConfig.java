...
/**
 * Persiste GEMINI_API_KEY en ~/.systag/gemini.properties
 * Prioridad: env var GEMINI_API_KEY > archivo > vacío (modo sin IA).
 */
public final class GeminiConfig {
    ...
    /** Resuelve key con prioridad env var > archivo. */
    public static synchronized String obtenerApiKey() {
        String env = System.getenv("GEMINI_API_KEY");
        if (env != null && !env.isBlank()) return env.trim();
        // property pasada como -D o System.setProperty
        ...
        String v = p.getProperty(KEY_PROP, "").trim();
    ...
    public static synchronized void guardarApiKey(String apiKey) {
        try {
            ...
            if (apiKey == null || apiKey.isBlank()) {
                ...
                p.setProperty(KEY_PROP, apiKey.trim());
            }
            try (FileOutputStream fos = new FileOutputStream(ARCHIVO)) {
                p.store(fos, "SysTagRep - Gemini API Key. Obtén gratis en https://aistudio.google.com/apikey\nPrioridad: env GEMINI_API_KEY > este archivo");
            }
