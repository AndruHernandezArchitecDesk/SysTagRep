Vendex 2.0 - Paquete PRODUCCION multi-PC
===========================================
...
CONTENIDO:
- comun/Vendex-2.0-SNAPSHOT.jar (44M, Java 17 + JavaFX embebido via shade)
...
- HOST: PostgreSQL 14+ con BD dbVendex
- Red LAN 192.168.1.0/24 accesible puerto 5432

INSTALACION HOST 192.168.1.7:
1) Copiar produccion/comun/Vendex-2.0-SNAPSHOT.jar a C:\Vendex\
...
2) Si no tienes MSI: generar MSI en Windows con empaquetar_instalador.bat
   (requiere JDK 17 + WiX 3.11) -> dist\Vendex-2.0.0.msi e instalar en ambas PCs.
   Si usas JAR: asegurar JDK 17 instalado.
3) Configurar PostgreSQL:
   - postgresql.conf: listen_addresses='*'
   - pg_hba.conf: host dbVendex app_vendex 192.168.1.0/24 scram-sha-256
   - CREATE USER app_vendex WITH PASSWORD 'CAMBIAR';
...
4) Ejecutar SQL 1 vez en HOST:
   psql -h localhost -U postgres -d dbVendex -f comun/sql/fix_multipc_numeracion_20260831.sql
...
5) Copiar firma .p12 a ambas: ej C:\Vendex\firma.p12 y seleccionar en app
   Firma ya viene en C:\Users\TU_USUARIO\.vendex\firma.properties (rutaP12 + clave cifrada)
6) Primera ejecucion crea C:\Users\TU_USUARIO\.vendex\db.properties con localhost (dejar asi)
   Licencia: activar solo en HOST con 449E9-2E789-B71DA-6D23A-20260930 para maquina emugoKABUsDQZhckLohKymLW
7) Ejecutar: run.bat o java -Xmx1024m --enable-native-access=ALL-UNNAMED -jar Vendex-2.0-SNAPSHOT.jar

INSTALACION CLIENTE 192.168.1.5:
1) Copiar mismo Vendex-2.0-SNAPSHOT.jar + run.bat a C:\Vendex\
   (o mismo MSI)
2) Copiar misma firma .p12 a misma ruta C:\Vendex\firma.p12
   + copiar C:\Users\HOST\.vendex\firma.properties y ambiente.properties a C:\Users\Cliente\.vendex\
3) Editar C:\Users\Cliente\.vendex\db.properties:
...
NOTAS:
- Para generar MSI en Windows: empaquetar_instalador.bat -> dist\Vendex-2.0.0.msi
  Ese MSI ya lleva JRE y no necesitas JDK en cliente.
- Etiquetas: 480x200 JPG en Documentos/etiquetas VENDEX/etiqueta_ID_COD_UBI_1de5.jpg
